...
# SETTINGS
# ─────────────────────────────────────────
STARTING_BALANCE    = 50.0    # Paper trading starting balance
BET_SIZE_PCT        = 0.05    # Bet 5% of balance per trade (conservative)
MIN_EDGE_PCT        = 5.0     # Only bet when edge is 5%+ over market odds
SCAN_INTERVAL       = 30      # Check markets every 30 seconds
MIN_TIME_REMAINING  = 60      # Don't enter with less than 60s left in window
...
class PaperTrader:
    ...
    def place_bet(self, market_id, direction, odds, true_prob, edge, stake):
        ...
        log.info(f"Paper bet placed: {direction} @ {odds:.2f} | stake ${stake:.2f} | edge {edge:.1f}%")
        return bet
    ...
    def settle_bet(self, bet_id, won: bool):
        for bet in self.open_bets:
            if bet["id"] == bet_id:
                ...
                log.info(f"Bet {bet_id} settled: {'WON' if won else 'LOST'} | profit ${profit:.2f}")
                return bet, profit
        return None, 0
...
async def get_btc_markets(session: aiohttp.ClientSession) -> list:
    """Fetch active 5-minute BTC up/down markets from Polymarket."""
...
async def get_btc_volatility(session: aiohttp.ClientSession) -> float:
    """Get recent BTC volatility (std dev of last 20 1-min candles)."""
    try:
        async with session.get(
        ...
        return 0.5  # Default 0.5% per minute volatility
...
def calculate_edge_with_claude(
    ...
    """Use Claude to calculate true probability and find betting edge."""
...
Seconds Remaining in Window: {seconds_remaining}s

MARKET ODDS (what Polymarket is offering):
UP odds (probability): {market_up_odds:.3f} ({market_up_odds*100:.1f}%)
DOWN odds (probability): {market_down_odds:.3f} ({market_down_odds*100:.1f}%)

YOUR TASK:
Calculate the TRUE probability of BTC ending UP vs DOWN at window close.

Use these factors:
1. Current momentum — if BTC is already up {price_change_pct:+.3f}%, it has a head start
2. Time remaining — with {seconds_remaining}s left and {volatility:.4f}% per min volatility, how much can it realistically move?
3. Mean reversion — does the current move suggest continuation or reversal?
4. The window ends UP if final price >= start price, DOWN if final price < start price

Calculate using a simplified random walk model:
- Expected move in remaining time = volatility * sqrt(seconds_remaining/60) 
- Given current position of {price_change_pct:+.3f}%, estimate probability of ending above 0%

Then compare your true probability to market odds to find the edge.

Return ONLY this JSON, no markdown:
{{
  "true_prob_up": <float 0-1>,
  "true_prob_down": <float 0-1>,
  "market_prob_up": {market_up_odds},
  "market_prob_down": {market_down_odds},
  "edge_up": <true_prob_up - market_prob_up, can be negative>,
  "edge_down": <true_prob_down - market_prob_down, can be negative>,
  "best_bet": "UP" or "DOWN" or "NO_BET",
  "best_edge": <the larger absolute edge as a positive float>,
  "reasoning": "2-3 sentence explanation of your calculation",
...
# TELEGRAM ALERTS
# ─────────────────────────────────────────
async def send_trade_alert(bot: Bot, trade: dict, analysis: dict, trader: PaperTrader):
    direction_emoji = "📈" if trade["direction"] == "UP" else "📉"
    msg = f"""{direction_emoji} *PAPER TRADE PLACED*

*Direction:* {trade['direction']}
*Market Odds:* {trade['odds']:.3f} ({trade['odds']*100:.1f}%)
...
💰 *Balance:* ${trader.balance:.2f} | *P&L:* {'+' if trader.pnl >= 0 else ''}${trader.pnl:.2f}"""
...
async def send_settlement_alert(bot: Bot, bet: dict, profit: float, trader: PaperTrader):
    emoji = "✅" if profit > 0 else "❌"
    msg = f"""{emoji} *TRADE SETTLED*

*Direction:* {bet['direction']} — *{'WON' if profit > 0 else 'LOST'}*
*Profit:* {'+' if profit >= 0 else ''}${profit:.2f}
*Stake was:* ${bet['stake']:.2f}

📊 *Stats*
├ Balance: ${trader.balance:.2f}
├ P&L: {'+' if trader.pnl >= 0 else ''}${trader.pnl:.2f}
├ Win Rate: {trader.win_rate:.0f}%
└ Trades: {trader.wins}W / {trader.losses}L"""
...
async def send_status(bot: Bot, trader: PaperTrader, btc_price: float):
    msg = f"""📊 *Bot Status Update*

*BTC Price:* ${btc_price:,.2f}
*Balance:* ${trader.balance:.2f}
*P&L:* {'+' if trader.pnl >= 0 else ''}${trader.pnl:.2f} ({trader.pnl/trader.start_balance*100:+.1f}%)
*Win Rate:* {trader.win_rate:.0f}%
*Trades:* {trader.wins + trader.losses} total ({trader.wins}W / {trader.losses}L)
...
class PolyBot:
    def __init__(self):
        ...
        self.tracked_markets = {}  # market_id -> {start_price, start_time, end_time}
        self.last_status = 0

    async def track_market(self, session, market):
        """Track an active market window."""
        ...
        try:
            ...
            log.info(f"Tracking market: {question[:50]} | {secs_remaining:.0f}s remaining | BTC ${btc_price:,.2f}")
        except Exception as e:
            log.warning(f"Track market error: {e}")

    async def evaluate_market(self, session, market_id, market_data):
        """Evaluate a tracked market and place paper bet if edge found."""
        ...
        # Get market odds from Polymarket
        # For simplicity use 50/50 as default if we can't get live odds
        market_up_odds   = 0.50
        ...
        try:
            ...
            if book:
                ...
                asks = book.get("asks", [])
                if bids:
                    market_up_odds = float(bids[0]["price"])
                    market_down_odds = 1 - market_up_odds
        except Exception:
            pass

        # Claude calculates edge
        try:
        ...
        best_edge = analysis.get("best_edge", 0) * 100  # Convert to percentage

        log.info(f"Analysis: {best_bet} | Edge: {best_edge:.1f}% | {secs_remaining:.0f}s left")

        if best_bet == "NO_BET" or best_edge < MIN_EDGE_PCT:
            log.info(f"No bet — edge {best_edge:.1f}% below minimum {MIN_EDGE_PCT}%")
            return

        if analysis.get("confidence") == "LOW":
            log.info("No bet — low confidence")
            return
    ...
    async def settle_expired_markets(self, session):
        ...
        for market_id, data in self.tracked_markets.items():
            if now >= data["end_time"]:
                ...
                # Get final BTC price to determine outcome
                ...
                won = (direction == "UP" and final_price >= start_price) or \
                      (direction == "DOWN" and final_price < start_price)
